resource "aws_ssm_association" "cloudwatch_agent" {
  ...
  parameters = {
    commands = join("\n", [
      ...
      "echo '${filebase64("${path.module}/../monitoring/cloudwatch/cloudwatch-agent.json")}' | base64 --decode > /opt/aws/amazon-cloudwatch-agent/etc/amazon-cloudwatch-agent.json",
